...
const DICT = {
  ...
  'settings.workroots.runnerNote': ['Claude·GLM·Kimi·Grok 크루는 도구 게이트가 폴더 단위로 적용하고, Argo 앱·자격 폴더는 항상 차단합니다. Codex·Gemini·Antigravity 크루는 프로세스 단위 샌드박스에 같은 폴더 목록이 그대로 전달됩니다 — 넓은 폴더(예: 홈 전체)를 지정하면 그 안의 보호 폴더까지 함께 열릴 수 있으니 필요한 폴더만 좁게 지정하세요. 이 러너들은 지정하지 않은 폴더는 아예 읽지도 쓰지도 못하며, 구버전 Gemini CLI는 벤더 제한으로 지정 폴더 확장이 무시될 수 있습니다. Kiro 크루는 예외입니다 — 벤더 CLI에 폴더 반경을 강제할 수단이 없어 지정 폴더가 반경으로 적용되지 않습니다. Argo 앱·자격 폴더와 다른 회사 데이터는 차단하지만, 그 밖의 이 컴퓨터 파일은 크루가 열 수 있습니다.', 'For Claude·GLM·Kimi·Grok crews the tool gate applies per folder and always blocks the Argo app and credential folders. Codex·Gemini·Antigravity crews receive the same folder list in their process-level sandbox — assigning a broad folder (e.g. your whole home) can open protected folders inside it too, so assign only what you need. These runners cannot read or write folders you have not assigned, and older Gemini CLI builds may ignore the expansion entirely (a vendor limit). Kiro is the exception — its CLI offers no way to enforce a folder radius, so assigned folders are not applied as one. The Argo app, credential folders and other companies\\u2019 data are blocked, but other files on this computer can be opened by the crew.'],
