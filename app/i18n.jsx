...
const DICT = {
  ...
  'settings.workroots.desc': ['크루가 회사 폴더 밖에서 일할 폴더를 지정합니다. 지정한 폴더는 크루가 우선적으로 안내받아 바로 읽고 쓰며, Antigravity 크루의 파일 도구 반경에도 더해집니다. 경로는 기기마다 달라 이 컴퓨터에만 적용됩니다.', 'Point crews at folders outside the company workspace. Assigned folders are surfaced to crews and immediately readable and writable, and are added to Antigravity crews’ file-tool scope. Paths are per-device, so this applies to this computer only.'],
  ...
  'chat.card.mcpCliWarn': ['이 크루의 러너(Antigravity)에서는 MCP 도구가 아직 실행되지 않습니다 — Codex·Gemini나 SDK 러너로 바꾸면 사용됩니다.', 'MCP tools do not run on this crew\'s runner (Antigravity) yet — switch to Codex, Gemini, or an SDK runner to use them.'],
