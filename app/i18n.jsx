...
const DICT = {
  ...
  'settings.msgr.help': ['아르고에 로그인하면 이 컴퓨터의 크루 전부가 팀 메신저의 내 조직에 파견됩니다. 조직 채널에서 @로 부르면 등록한 크루가 해당 대화에서 답합니다.', 'Signing in to Argo dispatches every crew on this computer to your team messenger organizations. Mention them with @ in an org channel and registered crews reply in that conversation.'],
  ...
  'settings.msgr.notify.title': ['크루 알림을 메신저로 받기', 'Crew notifications in Messenger'],
  'settings.msgr.notify.help': ['결재 요청·장시간 작업 완료·동료 쪽지 같은 크루 알림을 텔레그램처럼 메신저 방 하나로 받습니다. 방을 고르고 받을 알림을 켠 뒤 저장하세요. 알림은 해당 크루 이름으로 올라가며, 그 방의 구성원 모두가 내용을 봅니다(결재 사유·쪽지 본문 포함). 메신저 안에서 시작한 실행은 원래 방으로 갑니다.', 'Get crew notifications (approval requests, long task results, crew mail…) in one Messenger room, like Telegram. Pick a room, turn on the notifications you want, then save. They are posted under that crew\'s name and everyone in the room sees the contents (including approval reasons and crew mail). Runs started inside Messenger stay in their own room.'],
  'settings.msgr.notify.room': ['받을 방', 'Room'],
  'settings.msgr.notify.dm': ['{name}와의 1:1', '1:1 with {name}'],
  'settings.msgr.notify.active': ['알림 {n}종 켜짐', '{n} kinds on'],
  'settings.msgr.notify.eventsHint': ['끄면 메신저로만 안 갑니다 — 앱·텔레그램 알림은 그대로입니다.', 'Turning one off only stops Messenger — app and Telegram alerts stay as they are.'],
  'settings.msgr.notify.off': ['보내지 않음', 'Do not send'],
  'settings.msgr.notify.events': ['받을 알림', 'Notifications to receive'],
  'settings.msgr.notify.save': ['저장', 'Save'],
  'settings.msgr.notify.saved': ['저장했습니다.', 'Saved.'],
  'settings.msgr.notify.clear': ['끄기', 'Turn off'],
  'settings.msgr.notify.off.done': ['껐습니다.', 'Turned off.'],
  'settings.msgr.notify.noRooms': ['들어갈 수 있는 방이 없습니다 — 크루가 조직에 파견되어 있고 채널에 접근할 수 있어야 합니다.', 'No room available — a crew must be dispatched to the organization and have access to a channel.'],
  'settings.msgr.notify.err.save': ['알림 받을 방을 저장하지 못했습니다. 방 접근 권한과 메신저 로그인을 확인하세요.', 'Could not save the notification room. Check room access and the Messenger sign-in.'],
  ...
  'runner.external': ['외부 에이전트', 'External agent'],
  'agent.state.connecting': ['{label} 연결중', '{label} connecting'],
  'agent.state.ok': ['{label} 연결됨 · 마지막 응답 {ago}', '{label} connected · last reply {ago}'],
  'agent.state.fail': ['{label} 응답 없음 · {why}', '{label} no reply · {why}'],
  'deck.runner.external': ['{agents} 연결중 — 카드에 지정한 크루만 그 에이전트로 일합니다. 자동 러너 크루·새 영입에는 일반 러너 연결이 필요합니다.', '{agents} connecting — only crews assigned to them run there. Auto-runner crews and hiring need a regular runner.'],
