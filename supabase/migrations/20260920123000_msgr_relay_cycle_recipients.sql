...
begin
 ...
 if new.author_kind='crew' then
   ...
   -- The initiating crew has no prior relay marker but is already part of the chain.
   if via.id is not null and not (visited @> jsonb_build_array(to_jsonb(via.id::text))) then
     visited:=visited || jsonb_build_array(to_jsonb(via.id::text));
   end if;
 else
   depth:=1;
   chain_id:=new.id;
 ...
 for x in select value from jsonb_array_elements(coalesce(new.mentions,'[]')) loop
   ...
   if depth>5 or cycle then
     ...
     if depth>5 then return new; end if;
     -- A cyclic recipient must not suppress independent deliveries or their notice.
     continue;
   end if;
   begin
