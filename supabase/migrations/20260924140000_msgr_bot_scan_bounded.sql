-- 봇 스캔 범위 고정(2026-09-24 라이브 실측): 커서 뒤 글만 봐야 하는 배달 판정이 조직 글 전부에 불렸다.
-- 조건이 "커서 뒤 글 OR 위임받은 글(msgr_dm_grants)"이라 플래너가 비싼 msgr_delivery_allowed를 커서 조건보다 먼저 평가 →
-- 조직 글 443개 × 판정 = 봇당 714ms, 새 글 하나에 봇 11개가 동시에 돌아 CPU 포화 → 3초 statement timeout → 재시도 폭주(VPS 봇 첫 반응 30~50초).
-- 후보(커서 뒤 글 ∪ 위임받은 글) 행을 materialized CTE에 담고 판정은 그 행에만 한다(id만 담으면 조건이 원래 표 스캔으로 내려가 그대로였다). 본문은 20260923200000_msgr_bot_cursor_skip.sql 그대로 + 이 루프 머리만.
