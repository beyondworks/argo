...
-- 삽입 때만 now()로 덮는다 — 앱·브리지·SQL 어디도 created_at을 지정해 넣지 않는다(2026-09-24 전수 수색). 이 트리거는 update를 막지 않는다 — 인증된 사용자의 created_at 수정은 이미 msgr_lock_messages(msgr_immutable_created_at)가 막고, 서비스 문맥의 운영·테스트 시각 조정만 통과한다.
