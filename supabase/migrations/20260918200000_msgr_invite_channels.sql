...
create or replace function public.msgr_invite_redeem(p_code text) returns jsonb
  language plpgsql security definer set search_path = public, pg_temp as $$
declare inv public.msgr_invites%rowtype; ch uuid; joined uuid[] := '{}'; skipped uuid[] := '{}'; reuse boolean;
begin
  if auth.uid() is null then raise exception 'msgr_auth_required'; end if;
  ...
  -- 같은 사람이 같은 링크를 다시 열면 한 번 더 세지 않는다 — 단, 지금 유효한 멤버일 때만(쫓겨난 뒤 옛 1회용 링크로 되돌아오지 못하게)
  reuse := exists (select 1 from public.msgr_invite_uses u where u.invite_id = inv.id and u.user_id = auth.uid())
       and exists (select 1 from public.msgr_org_members m where m.org_id = inv.org_id and m.user_id = auth.uid() and m.removed_at is null and (m.expires_at is null or m.expires_at > now()));
  if not reuse and inv.max_uses is not null and inv.use_count >= inv.max_uses then raise exception 'msgr_invite_exhausted'; end if;
  ...
  -- 조직 멤버 등록(기존 규칙 그대로) — 이미 정식 멤버인 사람이 게스트 링크를 열면 강등하지 않는다(채널에만 넣는다)
  ...
  if not reuse then
    ...
    insert into public.msgr_invite_uses (invite_id, user_id) values (inv.id, auth.uid()) on conflict (invite_id, user_id) do update set used_at = now();
...
-- 이름 외(주제·멤버·글)는 없다. 틀린 코드는 msgr_invite_not_found 하나. 쓸 수 없는 초대는 {state, org_name}만.
...
begin
  ...
  return jsonb_build_object('state', st, 'org_id', inv.org_id, 'org_name', org_name, 'channels', chans, 'inviter_name', inviter, 'role', inv.role,
    'expires_at', inv.expires_at, 'max_uses', inv.max_uses, 'use_count', inv.use_count);
end $$;
