...
function Shell({ session }) {
  ...
  const dmLongPress = (c, items) => { const st = (lpStates.current[c.id] ??= { timer: null, x: 0, y: 0, el: null, firedAt: 0 });
    const { clear, ...lp } = longPressHandlers(st, () => { st.firedAt = Date.now(); if (!st.el) return;
      openCtx({ preventDefault() {}, stopPropagation() {}, currentTarget: st.el, clientX: st.x, clientY: st.y }, items, c.id); });
    void clear;
    // 손을 뗄 때 오는 click(메뉴 뒤 배경에 떨어져 메뉴를 닫고, 행에 떨어지면 대화를 연다)은 **pointerup 직후 300ms 안의 것만** 한 번 삼킨다 —
    // iOS가 click을 안 내는 경우(드래그 리프트·콜아웃)에 사용자의 다음 탭(메뉴 항목)을 먹지 않게(검수 HIGH-3)
    const swallowNext = () => { if (Date.now() - st.firedAt > 1500) return; const swallow = (e) => { e.preventDefault(); e.stopPropagation(); }; document.addEventListener('click', swallow, { capture: true, once: true }); setTimeout(() => document.removeEventListener('click', swallow, { capture: true }), 300); };
