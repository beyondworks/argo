...
import { createInvite, previewInvite, acceptInvite, revokeInvite, inviteStatus, daysLeft, inviteErrorKey, missingFn } from './invite-flow.mjs';
...
function Shell({ session }) {
  ...
  const createInviteCode = async (opts) => (await createInvite(supabase, { orgId, uid, ...opts })).code;
  ...
  return (
    ...
    <div className={`shell msgr-shell${rail ? ' rail-open' : ''}${isPhone ? ' msgr-phone' : ''}${isPhone && (page === 'home' || page === 'dm') ? ' phone-home' : ''}${isPhone && page === 'dm' ? ' phone-dm' : ''}${isPhone && pageAnim ? ` anim-${pageAnim}` : ''}`}>
      ...
      <main className="msgr-main" {...edgeBack}>
        ...
        {inviteFor && org && !isPersonal && <InviteDialog org={org} channels={inviteChannels} isAdmin={!!isAdmin} hostOf={hostChannels} initialChannelIds={inviteFor.channelIds} initialRole={inviteFor.role} create={createInviteCode} shareText={inviteShare} linkOf={inviteLinkOf} errorText={inviteErr} onClose={() => setInviteFor(null)} onManage={isAdmin ? manageInvites : null} t={t} phone={isPhone} />}
