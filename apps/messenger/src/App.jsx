...
function Shell({ session }) {
  ...
  useEffect(() => {
    if (!isPhone) return;
    const same = navPrev.current === page; navPrev.current = page; if (ROOT_PAGES.has(page)) lastRoot.current = page;
  ...
  const edgeBack = useEdgeSwipeBack(goBack, isPhone && page !== 'home' && page !== 'dm', { underlay: () => lastRoot.current }); // 폰: 왼쪽 가장자리 스와이프 = 뒤로(그 전 화면)
  ...
  useEffect(() => { // Realtime — 조직 topic 하나. 방송은 id·채널만 싣는다(본문은 RLS를 지난 조회로).
    ...
    const cleanup = realtimeScope.run(async (isDisposed, registerDispose) => {
      ...
      ch
        .on('broadcast', { event: 'message' }, active(({ payload }) => { if (payload?.author_user_id && payload.author_user_id === uid) mineRef.current.add(payload.id); setEvent({ kind: 'message', ...payload, at: Date.now() }); if (payload?.channel_id && dmIdsRef.current.has(payload.channel_id)) setLastAt((m) => ({ ...m, [payload.channel_id]: Date.now() })); if (payload?.channel_id && typeof payload.body === 'string' && dmIdsRef.current.has(payload.channel_id)) setLastMsg((m) => ({ ...m, [payload.channel_id]: { body: payload.body.replace(/\s+/g, ' ').trim().slice(0, 120), mine: payload.author_user_id === uid, at: Date.now() } })); if (!notifyMention(payload)) notifyReply(payload); })) // 멘션이면 멘션 알림 하나만
  ...
  useEffect(() => { // DM 최근순 재료 — 채널당 마지막 메시지 시각(RPC msgr_dm_latest, 채널당 1행·id 인덱스). 폰 DM 탭 정렬에만. 재연결·조직 전환 때도 다시.
    ...
    supabase.rpc('msgr_dm_latest', { org: orgId }).then(async ({ data }) => { if (!live || !data) return; const m = {}; for (const r of data) m[r.channel_id] = Date.parse(r.last_at); setLastAt((cur) => ({ ...cur, ...m }));
      ...
      const { data: rows } = await supabase.from('msgr_messages').select('id, channel_id, body, author_user_id, created_at').in('id', ids); if (!live || !rows) return;
      const lm = {}; for (const r of rows) lm[r.channel_id] = { body: String(r.body ?? '').replace(/\s+/g, ' ').trim().slice(0, 120), mine: r.author_user_id === uid, at: Date.parse(r.created_at) }; setLastMsg((cur) => ({ ...cur, ...lm })); }, () => {});
    return () => { live = false; };
  ...
  const [dmFilter, setDmFilter] = useState('all'); // 폰 DM 탭 상단 필터: 전체·즐겨찾기·안읽음·그룹(유건 2026-09-15, 슬랙 DM 페이지 대응)
  ...
  useLayoutEffect(() => {
    ...
    setChannels([]); setMembers([]); setCrews([]); setMyAvailable([]); setLastAt({}); setChId(null); setChMembers([]); setDmMembers({}); setEnt(null); setPolicy(null); setUnread({}); setBotKinds([]);
  ...
  const dmIsGroup = (c) => (dmMembers[c.id] ?? []).length > 2;
  ...
  const dmList = dmTab ? dmSorted(dmPool.filter(dmVisible)) : dms; // dms는 이미 고정 제외 — 전체 탭의 고정은 아래 별도 단락(유건 2026-09-15)
  ...
  const dmLongPress = (c, items) => { const st = (lpStates.current[c.id] ??= { timer: null, x: 0, y: 0, el: null, firedAt: 0 });
    const lp = longPressHandlers(st, () => { st.firedAt = Date.now(); if (!st.el) return;
      // 손을 뗄 때 오는 click(메뉴 뒤 배경에 떨어져 메뉴를 닫고, 행에 떨어지면 대화를 연다)을 한 번 삼킨다 — 2초 안 안 오면 해제
      const swallow = (e) => { e.preventDefault(); e.stopPropagation(); }; document.addEventListener('click', swallow, { capture: true, once: true }); setTimeout(() => document.removeEventListener('click', swallow, { capture: true }), 2000);
      openCtx({ preventDefault() {}, stopPropagation() {}, currentTarget: st.el, clientX: st.x, clientY: st.y }, items, c.id); });
    return { ...lp, onPointerDown: (e) => { st.el = e.currentTarget; lp.onPointerDown(e); } }; };
  const dmRow = (c) => { const dmMs = dmMembers[c.id] ?? []; const dmCrew = dmMs.find((m) => m.member_kind === 'crew'); const dmOther = dmMs.find((m) => m.member_kind === 'user' && m.member_id !== uid); const withCrew = !!dmCrew; const confirmVia = (kind) => { setActionError(''); setRailAction({ channel: c, kind }); }; const items = [
            ...
            <div key={c.id} className={`msgr-railrow${ctx?.trigger === c.id ? ' open' : ''}${drag === c.id ? ' dragging' : ''}`} draggable onDragStart={dragStart(c)} onDragEnd={() => setDrag(null)} onDragOver={dragOver} onDrop={(e) => dropOnRow(e, c)} onContextMenu={(e) => { if (Date.now() - (lpStates.current[c.id]?.firedAt ?? 0) < 800) { e.preventDefault(); return; } openCtx(e, items, c.id); }} {...(dmTab ? dmLongPress(c, items) : {})}>
              <button type="button" className={`item${c.id === chId ? ' active' : ''}${unread[c.id]?.n && !muted.has(c.id) ? ' unread' : ''}`} onClick={() => { setChId(c.id); setRail(false); if (isPhone) setPage('chat'); setPage('chat'); }}><Av name={dmName(c)} size="xs" crew={withCrew} crewId={dmCrew?.member_id ?? null} userId={dmCrew ? null : (dmOther?.member_id ?? null)} />{dmTab ? <span className="dmtext"><span className="dmline"><span className="name">{dmName(c)}</span>{lastMsg[c.id]?.at > 0 && <span className="when">{fmtDmWhen(lastMsg[c.id].at, lang)}</span>}</span>{lastMsg[c.id]?.body && <span className="snip">{lastMsg[c.id].mine ? t('dm.snip.me') : ''}{lastMsg[c.id].body}</span>}</span> : <span className="name">{dmName(c)}</span>}{unread[c.id]?.n > 0 && <span className={`msgr-badge${muted.has(c.id) ? ' dim' : ' mark'}`}>{unread[c.id].n}</span>}</button>
              <button type="button" className="more" onClick={(e) => { openCtx(e, items, c.id); }} title={t('ch.row.more')} aria-label={t('ch.row.more')} aria-haspopup="menu" aria-expanded={ctx?.trigger === c.id}><I name="dots" size={13} /></button>
  ...
  return (
    ...
    <div className={`shell msgr-shell${rail ? ' rail-open' : ''}${isPhone ? ' msgr-phone' : ''}${isPhone && (page === 'home' || page === 'dm') ? ' phone-home' : ''}${isPhone && page === 'dm' ? ' phone-dm' : ''}`}>
      ...
      {dmPeek && <DmPeekSheet channel={dmPeek} name={dmName(dmPeek)} uid={uid} onOpen={() => { const c = dmPeek; setDmPeek(null); setChId(c.id); setRail(false); setPage('chat'); }} onClose={() => setDmPeek(null)} />}
...
function DmPeekSheet({ channel, name, uid, onOpen, onClose }) {
  const { t, lang } = useT();
  const [rows, setRows] = useState(null);
  useEffect(() => { let live = true; supabase.from('msgr_messages').select('id, body, author_kind, author_user_id, created_at').eq('channel_id', channel.id).is('deleted_at', null).order('id', { ascending: false }).limit(12)
      ...
      <section className="msgr-crewsheet msgr-dmpeek" role="dialog" aria-label={t('dm.preview')}>
        ...
        <div className="peek">
          {rows === null ? <p className="note">{t('ui.loading')}</p> : !rows.length ? <p className="note">{t('dm.preview.empty')}</p> : rows.map((m) => (
            <div key={m.id} className={`pk${m.author_user_id === uid ? ' me' : ''}`}><span className="who">{m.author_user_id === uid ? t('dm.snip.me').trim() : (m.author_kind === 'crew' ? name : t('dm.preview.other'))}</span><span className="body">{m.body}</span><span className="when">{fmtDmWhen(Date.parse(m.created_at), lang)}</span></div>
          ))}
        </div>
      </section>
    </div>
...
function Message({ m, uid, lang, t, nameOfUser, crewOf, isAdmin, policy, ap, atts, decide, parent, onCrew, onError, reacts = [], onReact, onEdit, onDelete, channels = [], onOpenRelay, dmName }) {
  ...
  const acts = !ap && !m.deleted_at && !editing && (
    ...
    <div className={`msgr-acts${ctxAt && actsOpen ? ' ctx' : ''}`} style={ctxAt && actsOpen ? { left: Math.max(4, Math.min(ctxAt.x, window.innerWidth - 200)), top: Math.max(4, Math.min(ctxAt.y, window.innerHeight - 180)) } : undefined}>
