...
export function Graph3D({ docs, agents = [], focusRel = null, onSelectDoc, compact = false, height = '100%', hint = '' }) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      if (!compact) { ctx.beginPath(); const cy = Math.cos(cam.yaw), sy = Math.sin(cam.yaw), cp = Math.cos(cam.pitch), sp = Math.sin(cam.pitch); const F = (cam.dist * Math.min(W, H) * 0.42) / radius;
