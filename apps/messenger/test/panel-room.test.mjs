...
test('패널 폭과 비킴 폭이 같다 — 패널 규칙이 바뀌면 비킴 수치도 같이 바뀌어야 한다', () => {
  assert.match(css, /\.msgr-crewsheet \{ position: absolute; top: 72px; right: 24px; width: min\(380px,/, '패널 폭 380 + 오른쪽 24');
...
test('데스크톱에서만, 열린 패널에만 — 폰 시트와 아래에서 뜨는 dmpeek는 제외', () => {
  const rules = css.split('\n').filter((l) => l.includes('.msgr-sheetwrap > .msgr-crewsheet'));
