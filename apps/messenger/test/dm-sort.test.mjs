...
test('배선 — DM 탭에서만 고정 DM을 맨 위에 + 정렬 메뉴, 즐겨찾기는 DM 탭에서도 사라지지 않는다', () => {
  ...
  assert.match(src, /function DmGroupSheet\(/, '새 그룹 대화 시트'); assert.match(src, /const createGroupDm = async \(picks\) =>/, '그룹 생성'); assert.match(src, /page !== 'dm' \? setNewCh\(\{ name: '', kind: 'public' \}\) : isPersonal \? \(setPage\('settings'\), setSettingsTab\('friends'\)\) : setDmGroup\(true\)/, 'DM 탭의 +는 그룹 대화(개인 공간은 친구 화면 — 행동은 personal-space.browser.mjs)');
