...
test('DM 일지는 상대 이름으로 표기(dmName 전달)', () => {
  ...
  assert.match(app, /const findCh = \(id\) => channels\.find\(\(c\) => c\.id === id\) \?\? previewChannels\.find\(\(c\) => c\.id === id\) \?\? chiefChannel\(id\);/, '참여 안 한 공개 채널 일지도 #이름(참여 기준 #555 뒤 channels에 없음), 장 전용 채널은 RPC가 준 이름 — 행동은 memory-journal.browser.mjs');
