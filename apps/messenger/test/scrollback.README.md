...
| `scrollback.browser.mjs` | 아무 곳(에이전트가 실행) | ego-browser 행동 검사 5종 |
| `mutations.mjs` | 아무 곳 | 변이 주입기(핀이 행동을 잠그는지 실증) |
...
# 2) 검사 (ego-browser)
ego-browser nodejs < scrollback.browser.mjs        # 게이트 실패 시 종료코드 1
SB_STRICT=1 ego-browser nodejs < scrollback.browser.mjs   # 결함재현 실패도 종료코드에 포함

# 3) 변이로 잠금 실증
node mutations.mjs apply M4 apps/messenger/src/App.jsx   # → 게이트 1 이 red 여야 한다
ego-browser nodejs < scrollback.browser.mjs
node mutations.mjs revert apps/messenger/src/App.jsx
```
...
아래 '실증' 열은 23c67513 에서 직접 돌려 본 결과다(추정 아님).
...
| 게이트 3 · 끝에 닿으면 '대화의 시작' | 더 있음 판정 | **M6** 미확인(실행이 환경 정리에 잘림) |
| 결함재현 1(HIGH-1) · 첨부 hydrate 착지 뒤에도 0px | 늦게 오는 높이 변화까지 되맞춤 | **M10**(되맞춤 두 갈래 모두 제거) ✅ · **M2** ✅ |
| 결함재현 2(HIGH-2) · 모바일 재개 뒤 기록 유지 | 재개가 목록을 갈아엎지 않음 | **M8**(전체 교체로 되돌림) 미확인 |
...
2. **바닥 고정 해제는 진짜 휠 입력으로만 된다.** 합성 `WheelEvent` 는 앱의 제스처 표시에 닿지 않는다(합성 휠 + `scrollTop` 대입 뒤 내용을 키우면 그대로 바닥으로 끌려가는 것으로 확인). 그래서 휠은 가벼운 첫 페이지에서만 쓰고, 실제 로드는 버튼 클릭으로 한다 — 스레드가 무거워지면 CDP 입력이 타임아웃으로 죽는다.
...
## 마지막 실행 결과 (2026-09-14, `23c67513` = #531 머리 + 검수 반영 커밋)
