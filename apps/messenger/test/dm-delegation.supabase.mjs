...
function query(table){let op='select',values,cols='*',one=false;const filters=[];const api={
 ...
 if(op==='upsert'||op==='insert'){rows=(Array.isArray(values)?values:[values]).map(v=>{const keys=table==='msgr_target_prefs'?['user_id','org_id','target_kind','target_id']:['user_id','channel_id'];const old=op==='upsert'?all.find(r=>keys.every(k=>r[k]===v[k])):null;if(old){Object.assign(old,v);return old;}const row={...v,...(table==='msgr_messages'?{id:Math.max(101,...all.map(r=>r.id||0))+1,created_at:now,kind:'text',deleted_at:null}:{})};all.push(row);return row;});}
...
export const supabase={from:query,auth:{getSession:async()=>({data:{session:{user:{id:uid,email:'fixture@example.invalid'},access_token:'fixture'}}}),onAuthStateChange:()=>({data:{subscription:{unsubscribe(){}}}})},
 rpc:async(name,args)=>result({rpc:name,args},()=>{if(name==='msgr_dm_candidates')return [...state.tables.msgr_crews.map(c=>({...c,delivery_protocol:state.readiness?.[c.id]===false?0:1,delivery_ready:state.readiness?.[c.id]!==false})),{id:'crew-outdated',display_name:'Fixture Outdated Agent',delivery_protocol:0,delivery_ready:state.upgraded===true},{id:'crew-unknown',display_name:'Fixture Unknown Agent'}];if(name==='msgr_create_channel'){const id=`created-dm-${state.tables.msgr_channels.length}`;state.tables.msgr_channels.push(channel(id,args.kind,args.name));state.tables.msgr_channel_members.push({channel_id:id,member_kind:'user',member_id:uid},...args.others.map(m=>({channel_id:id,member_kind:m.kind,member_id:m.id})));return id;}if(name==='msgr_leave_dm'){state.tables.msgr_channels=state.tables.msgr_channels.filter(c=>c.id!==args.ch);return true;}return [];}),
