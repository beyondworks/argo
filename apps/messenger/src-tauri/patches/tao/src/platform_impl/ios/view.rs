...
pub fn create_delegate_class() {
  ...
  extern "C" fn configuration_for_connecting_scene_session(
    ...
    unsafe {
      ...
      // Argo 패치(2026-09-17): as_ptr는 함수 끝에서 config가 해제된 뒤의 포인터를 돌려줘 릴리스 빌드에서 UIKit이 해제된 객체를 retain → EXC_BAD_ACCESS
      // (TestFlight 0.1.27.2 충돌). tao 0.36.0과 같은 수정 — 자동 해제 풀로 넘겨 반환한다.
