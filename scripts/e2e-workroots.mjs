// 외부 작업 폴더(work roots, #137) E2E — 격리 서버 + codex 실 LLM으로 "fs ON + work root 등록 →
...
const PORT = 3197;
const ROOT = await mkdtemp(join(tmpdir(), 'argo-e2e-wr-'));
...
{
  const r = await api('/api/companies', { method: 'POST', body: { name: 'e2e-workroots', lang: 'ko' } });
...
const ask = () => api(`/api/companies/${WS}/chat`, {
  method: 'POST',
  body: { slug: CREW, message: `쓰기 테스트다. 파일 ${TARGET} 에 내용 "workroots-ok"를 저장하라. 성공/실패만 한 줄로 보고하라.` },
});
// ① 대조군 — 허용 폴더 미등록: 홈 밖(그리고 workRoots 밖)이므로 실패해야 한다.
...
// ② 허용 폴더 등록 후 — 같은 지시가 성공해야 한다
{
  ...
  if (!(c.json?.roots ?? []).length) fail(`work root 등록 실패: ${c.text.slice(0, 150)}`);
  console.log(`[e2e] 외부 작업 폴더 등록: ${c.json.roots.join(', ')}`);
  const r = await ask();
  const written = await readFile(TARGET, 'utf8').catch(() => null);
  if (written === null || !written.includes('workroots-ok')) {
    fail(`등록 후에도 홈 밖 쓰기 실패 — 크루 답변: ${(r.json?.reply ?? r.text).slice(0, 200)}`);
  }
  console.log(`[e2e] 홈 밖 쓰기 실측 성공: ${TARGET} → "${written.trim()}"`);
}
console.log('E2E OK: fs ON 단독=차단 유지, work root 등록=홈 밖 쓰기 성공 (main #137 codex writable_roots 실측)');
cleanup(0);
