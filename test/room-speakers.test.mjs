...
test('영어 회사는 영어 안내 줄', async () => {
  ...
  assert.equal((await loadRoom('rs-en')).messages[1].text, '2 speak at once — 비스트, 울프 · then a reaction round', '기본 = 반응 라운드 포함(유건 결정)');
  await seed('rs-en-relay', { lang: 'en' });
  await runRoomTurn('rs-en-relay', '@beast > @wolf go');
  assert.equal((await loadRoom('rs-en-relay')).messages[1].text, '2 speak in turn — 비스트, 울프', '릴레이는 순차 문구');
});
...
test('회의록 참석자에 system이 섞이지 않는다 — 안내 줄이 2명 이상 회의마다 붙어 상시화됐다(검수 MEDIUM-1)', async () => {
  ...
  await runRoomTurn('rs-minutes', '@비스트 @울프 @슈리 @에드나 출시 점검', [], { rounds: 1 });
...
test('릴레이에서 발언자 실패로 루프가 끊기면 차례가 오지 않은 크루를 밝힌다 — "N명 발언" 약속의 회수(검수 MEDIUM-2). 동시 발언의 실패는 room-parallel', async () => {
  const ok = stub.state.reply;
  stub.state.reply = (slug) => { if (slug === 'shuri') throw new Error('runner boom'); return `${slug} 답변`; };
  try {
    await seed('rs-fail');
    await assert.rejects(runRoomTurn('rs-fail', '@비스트 > @슈리 > @에드나 해줘'), /runner boom/, '호출 탭의 오류 계약은 그대로');
