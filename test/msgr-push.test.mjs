...
test('페이로드 — APNs 스레드/충돌 키·FCM 데이터에 채널·메시지 id가 문자열로 실린다', () => {
  ...
  assert.equal(a.aps.alert.title, 't'); assert.equal(a.aps['thread-id'], 'c1'); assert.equal(a.message_id, '7'); assert.equal(a.aps.sound, 'wood-knock.caf', '기본 소리');
  assert.equal(apnsPayload({ title: 't', body: 'b', channelId: 'c', messageId: 1, sound: 'wood-knock' }).aps.sound, 'wood-knock.caf');
  assert.equal(apnsPayload({ sound: 'seatbelt-single' }).aps.sound, 'seatbelt-single.caf', 'existing explicit choice stays');
  for (const sound of [null, '', undefined]) assert.equal(apnsPayload({ sound }).aps.sound, 'wood-knock.caf');
