...
// 부재중 판정은 전부 90초(앱 AWAY_MS·work_runs·알림 3분) — 60초 넘은 행만 갱신해도 최대 75초라 표시가 바뀌지 않는다.
import test from 'node:test';
import assert from 'node:assert/strict';
import { makeDb } from '../src/gateway/msgr.mjs';

test('심박은 60초 넘게 지난(또는 없는) 행만 갱신한다', async () => {
  const seen = [];
  ...
  assert.ok(Math.abs(before - 60_000 - cutoff) < 2_000, '기준 60초');
