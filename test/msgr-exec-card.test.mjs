...
test('P0: 안 읽음 RPC → 레일 배지(멘션은 mark·음소거는 dim)·굵은 이름·새 메시지 구분선(열 때 커서 고정)·보는 채널은 커서 갱신 · 편집/삭제는 본인 hover 액션 · 반응 칩·피커 · 음소거 메뉴·헤더 표시 · 조용한 시간은 알림 게이트', () => {
  ...
  assert.doesNotMatch(app, /const quick = topEmoji\(3\);|className="quick"/, 'hover 추천 없음(유건 지시)'); assert.match(app, /grid\(topEmoji\(COLS\), 'freq'\)/, '자주 사용은 피커 상단 한 줄(열 수만큼)'); assert.match(app, /function EmojiPicker\(\{ t, anchor, onPick, onClose \}\)[\s\S]*?searchEmoji\(q\)[\s\S]*?EMOJI_GROUPS\.map/, '슬랙식 피커: 검색·자주 사용·분류'); assert.match(app, /className="msgr-emojipop" ref=\{ref\}[^\n]*style=\{\{ left, top, width: W, maxHeight: H \}\}/, '피커는 화면 고정(스크롤 무관)'); assert.match(read('apps/messenger/src/styles.css'), /^\.msgr-emojipop \{ position: fixed;/m, 'fixed'); assert.match(app, /document\.body\.classList\.add\('msgr-lock'\)/, '열린 동안 스크롤 잠금'); assert.match(read('apps/messenger/src/styles.css'), /^body\.msgr-lock \.msgr-thread \{ overflow-y: hidden; \}/m, '잠금 CSS');
