...
test('⑤ chat.mjs: 폴더 조회 2곳(SDK·CLI) 모두 workFolder를 넘기고, 재시도 재귀 6곳이 workFolder를 잃지 않는다', async () => {
  ...
  assert.match(src, /makeCrewServer\(wsId, agentSlug, meta\.name \|\| agentSlug, colleagues, hop, chain, mirrorCtx, lang, connectors, workFolder, crewSink, journal\)/, '크루 서버 생성이 workFolder를 안 넘긴다'); // crewSink = 네이티브 엔진 도구 정의 sink(P-A)
});
