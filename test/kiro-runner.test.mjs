...
test('답변 추출 폴백 — 도구 진단(거부 파일 diff)이 답변으로 새지 않는다', () => {
  ...
  // ⚠ 트레이드오프(의도된 동작): 추적 줄 제거를 **항상** 적용하므로, 답변 본문이 kiro식 번호 diff
  // (`+` + 공백 2칸 이상 + 숫자 + `:`)를 그대로 담고 있으면 그 줄도 지워진다. 번호 diff는
  // fs_write가 **거부 전에** 기존 파일 내용을 렌더하는 자리라 유출 위험이 가장 큰 규칙이고,
  // 그걸 살리는 대가로 이 좁은 산문 손실을 받아들였다(설계 문서 한계 절에 명시).
