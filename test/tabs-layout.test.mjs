...
test('설정 — 카드 19개가 정확히 한 탭에만, 순서는 작은 카드 → 전폭(.wide)', async () => {
  ...
  const cards = {
    general: ["onSubmit={saveName}", "t('settings.spec')", '<LanguageCard />', '<CrewLanguageCard', '<ZoomCard />', '<ThemeCard />', '<LocalAssetImport'],
    ai: ['<AiConnectionCard'],
  ...
  assert.equal(Object.values(cards).flat().length, 23, '카드 목록 점검용 상수(19 카드 + 회사 정보·제원·보관 = 23 항목)');
  ...
  for (const solo of ['<NotifyChannelsCard', 'kind="telegram"', 'kind="slack"', '<MsgrCard', '<ConnectorsCard', '<WorkRootsCard', '<SyncCard', '<E2eeCard />', '<ImportCard', '<ThemeCard />', '<AiConnectionCard']) {
  ...
  assert.equal((src.match(/<div className="cardrow">/g) ?? []).length, 17, '행 수: 일반 4·AI 1·연결 5(알림 받을 메신저·텔레그램·슬랙·메신저·커넥터)·기기 6·위험 1');
