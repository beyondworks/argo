...
test('설정 — 카드 20개가 정확히 한 탭에만, 순서는 작은 카드 → 전폭(.wide)', async () => {
  ...
  const cards = {
    general: ["onSubmit={saveName}", "t('settings.spec')", '<LanguageCard />', '<CrewLanguageCard', '<ZoomCard />', '<FullAutoCard', '<ThemeCard />', '<LocalAssetImport'],
    ai: ['<AiConnectionCard'],
    connections: ['<NotifyChannelsCard', 'kind="telegram"', 'kind="slack"', '<MsgrCard', '<ConnectorsCard'], // MsgrCard = 팀 메신저 조직 파견(브리지 합류 2026-09-06), NotifyChannelsCard = 알림 받을 메신저 체크박스(유건 결정 2026-09-15)
  ...
  assert.equal(Object.values(cards).flat().length, 24, '카드 목록 점검용 상수(20 카드 + 회사 정보·제원·보관 = 24 항목, 풀 오토 모드 카드 2026-09-26 추가)');
  ...
  for (const solo of ['<NotifyChannelsCard', 'kind="telegram"', 'kind="slack"', '<MsgrCard', '<ConnectorsCard', '<WorkRootsCard', '<SyncCard', '<E2eeCard />', '<ImportCard', '<FullAutoCard', '<ThemeCard />', '<AiConnectionCard']) {
  ...
  assert.equal((src.match(/<div className="cardrow">/g) ?? []).length, 18, '행 수: 일반 5(회사정보·언어묶음·풀오토·테마·가져오기)·AI 1·연결 5(알림 받을 메신저·텔레그램·슬랙·메신저·커넥터)·기기 6·위험 1');
