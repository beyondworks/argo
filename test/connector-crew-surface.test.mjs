...
test('배선 — SDK 턴이 턴 요약을 크루 서버와 프롬프트 양쪽에 넘긴다(소스 고정)', async () => {
  ...
  assert.match(src, /mirrorCtx, lang, connectors, workFolder, journal\)/, '크루 서버에 요약이 전달되지 않는다'); // workFolder = 회의 폴더(위임 턴 전달, room-work-folder 테스트가 잠금), journal = 팀 메신저 일지 정책(msgr-bridge 테스트가 잠금)
