...
test('배선 — SDK 턴이 턴 요약을 크루 서버와 프롬프트 양쪽에 넘긴다(소스 고정)', async () => {
  ...
  assert.match(src, /mirrorCtx, lang, connectors, workFolder, crewSink, journal, fullAuto\)/, '크루 서버에 요약이 전달되지 않는다'); // crewSink = 네이티브 엔진이 같은 도구 정의를 받는 sink(하네스 통일 P-A) // workFolder = 회의 폴더(위임 턴 전달, room-work-folder 테스트가 잠금) // fullAuto = 풀 오토 모드(회사 단위 스위치, 유건 확정 2026-09-26)
