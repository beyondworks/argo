...
// 블록 주석 분기는 **같은 줄 닫힘(*/) + 뒤는 공백만**을 강제한다 — `//`와 달리 블록 주석은 줄
// 중간에 닫히고 뒤에 실제 코드가 올 수 있어, `\/\*.*`처럼 줄 끝까지 삼키면 닫힌 주석 뒤의 다음
// 항목이 조용히 검사에서 빠진다(#360 2R 실측 — 이 파일이 없애려는 바로 그 fail-open의 재생산).
// 알려진 한계(#360 검수 조건 1 — 아래 '알려진 한계' 테스트가 실행 문서): 값 문자열 **안에**
// `], //`가 들어 있는 병적 항목은 조기 종결되는데, 그 앞의 완결 토큰 수에 따라 방향이 갈린다 —
// 값 2개(정상 형식)면 0개로 오판돼 red(fail-closed)지만, 값 3개 이상(형식 위반)이면 앞의 2개만
// 세어져 **통과할 수 있다**(옛 정규식은 3개로 적발했다 — 이 병적 형태에 한해 탐지력 후퇴).
// 이 한계는 `//` 형태 한정이다 — `], /*` 값 내 함정은 닫힘 강제 덕에 red(3)로 적발된다.
const DICT_ENTRY_RE = /^\s*'([^']+)':\s*\[([\s\S]*?)\],?\s*(?:\/\/.*|\/\*.*\*\/\s*)?$/gm;
...
test('검사기: 후행 블록 주석(/* */)도 같은 기전 — 다음 항목이 삼켜지지 않는다 (#360 검수 조건 2)', () => {
  ...
  // 줄 중간에서 닫힌 블록 주석 뒤에 항목이 이어지는 형태 — `\/\*.*`(닫힘 비강제)였다면 x.b가
  // 스캔에서 조용히 사라졌다(#360 2R 실측). red(값 4)는 오판이지만 눈에 띄는 안전한 방향.
  const sameLine = scanDictEntries("  'x.a': ['하나', 'one'], /* c */ 'x.b': ['둘'],");
  assert.equal(sameLine.length, 1, '전제: 한 줄이라 항목 하나로 읽힌다');
  assert.notEqual(sameLine[0][1], 2, '닫힌 주석 뒤 항목이 주석으로 삼켜져 GREEN이 되면 형식 위반이 은폐된다');
});
...
test('검사기 알려진 한계: 값 안의 "], //"는 완결 토큰 수에 따라 방향이 갈린다 (#360 검수 조건 1 — 실행 문서)', () => {
  ...
  // 값 3개(형식 위반 — 이 검사가 잡아야 할 대상) + 셋째 값 안에 `], //` 함정 → 앞의 2개만 세어져
  // **통과**. 옛 정규식은 3개로 적발했다 — 이 병적 형태에 한해 탐지력이 후퇴함을 실행 문서로
  // 박아둔다. 이 단언이 red로 바뀌었고 그때 값이 3으로 세어진다면 그것은 **개선**이다 — 단언을
  // 3으로 고치고 위 한계 서술을 지울 것(틀린 값 2는 고착이 아니라 현재 한계의 가시화다).
  assert.deepEqual(scanDictEntries("  'p.three': ['x', 'y', 'z ], // 함정'],"), [['p.three', 2]],
    '3값+값 내 함정은 현 정규식이 2개로 세어 통과한다 — 문서화된 한계(자연스러운 후행 주석 전체를 고치는 대가)');
  // 같은 함정의 `/* ` 변종은 한계가 **아니다** — 블록 주석 닫힘 강제 덕에 red(3)로 적발된다(#360 2R).
