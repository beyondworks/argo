...
import { snapshotArtifacts, diffArtifacts, servableArtifact, SERVE_PREFIXES } from '../src/artifacts.mjs';
...
test('배선: chat() 두 반환부가 diff를 실고, 호출부 7곳이 artifacts를 전파한다', async () => {
  const chat = await readFile(new URL('../src/chat.mjs', import.meta.url), 'utf8');
  // SDK·CLI 반환부 둘 다 — CLI 미수집은 러너별 편파였다(중립성 원칙).
  assert.equal(chat.split('diffArtifacts(artBefore').length - 1, 2, 'SDK·CLI 반환부 각각 diff');
