...
test('점검 2026-09-12 소형 결함 4건: 읽음은 초점 있을 때만 · 사람 DM·답글도 알림 · 독 배지 음소거 제외 · 이미지 첨부 인라인', () => {
  ...
  assert.match(app, /<button type="button" ref=\{imgBtn\} className="msgr-imgbtn" aria-label=\{a\.name\} onClick=\{\(\) => setZoom\(true\)\}><img className="msgr-imgprev" src=\{src\} alt="" loading="lazy" onError=\{\(\) => setImgFail\(true\)\} \/><\/button>/, '이미지 인라인 — 누르면(키보드 포함, K10) 그 자리에서 확대(유건 2026-09-16), 못 그리면 파일 칩으로(D21)');
