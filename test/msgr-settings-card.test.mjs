...
test('I-3: 채널 개인 크루 정책 — 조회·시트 세그먼트(dm 제외)·차단 안내·멤버 추가 거절 문구·멘션 후보 필터, 브리지는 사유 RPC(채널 포함)로 묻고 채널 사유를 안내, 서버 게이트 3종', () => {
  ...
  assert.match(ch, /&& \(isDmRoom \? \(c\.owner_user_id === uid \|\| userIds\.has\(c\.owner_user_id\)\) : \(c\.owner_user_id === uid \|\| \(!!org\?\.service_user_id && c\.owner_user_id === org\.service_user_id\)\)\)\);/, '후보: 채팅=주인이 그 방에 있는 에이전트, 그 밖=방장이어도 내 에이전트와 조직 서비스 계정의 회사 에이전트만(유건 2026-09-17 — 남이 연결한 봇 포함 제외). 행동은 apps/messenger/test/dminvite.browser.mjs host-adds-and-approves');
  ...
  assert.match(bridge, /body: denyBody\(why, crew, lang\),/, '거절 안내가 사유별 문구 표를 쓰지 않는다');
