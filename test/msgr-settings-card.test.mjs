...
test('스크롤 QA(2026-09-04): 스레드는 바닥 고정 ref + ResizeObserver(렌더 뒤 높이 변화 추적)·위로 올려두면 유지, 채널 전환 시 바닥부터; 레일은 railbody만 스크롤(풋터 고정)', () => {
  ...
  assert.match(app, /<div className=(?:"msgr-railbody"|\{`msgr-railbody[^\n]*?`\})(?: ref=\{\w+\.setRef\})?(?: \{\.\.\.\w+Swipe\})?>(?:<PullIndicator [^>]*\/>)?<div className="msgr-railinner">[\s\S]{0,900}?<RailSection id=\{orgId \? 'channels' : 'start'\} label=\{orgId \? t\('ch\.list'\) : t\('org\.start'\)\}/, '레일 본문 스크롤 영역 안의 내용 래퍼(2026-09-14 railinner: 폰에서 min-height 100%+1px로 짧은 목록도 iOS 바운스) → 채널 절');
  const css = read('apps/messenger/src/styles.css');
