...
test('D3. 배선 핀 — chat.mjs가 modelFallback을 두 반환 경로에 싣고, 두 catch가 surfaceRunnerFailure를 지난다', async () => {
  ...
  assert.match(route, /failedCode, failedOrigin, aborted, cancellationIncomplete, viaSendNow, attachments \}/, '실패 코드와 중단 확인 상태가 스레드에 기록');
...
test('R2·R3·R9·MEDIUM-3 배선 핀 — 이벤트·라우트 응답·UI 렌더·모델 소비자', async () => {
  ...
  const route = await readFile(join(ROOT, 'app', 'api', 'companies', '[ws]', 'chat', 'route.js'), 'utf8');
  assert.match(route, /Response\.json\(\{ error: failed, code: failedCode, origin: failedOrigin, aborted, cancellationIncomplete, viaSendNow, saved \}/, 'R3: 500 응답 code/origin 및 중단 확인 상태');
