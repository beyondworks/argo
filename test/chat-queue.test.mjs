...
test('실패·중단 턴은 대기열을 잠근다(자동 발사 금지) + 푸는 자리가 있다', () => {
  ...
  // 잠금을 푸는 자리는 배출 이펙트·버튼·지금 바로 보내기(성공 시) 셋뿐이어야 한다(sendMessage
  // 진입부에서 풀면 무관한 지시 하나로 대기열 전체가 발사된다 — M-1)
  assert.doesNotMatch(send.slice(0, send.indexOf('} catch (err) {')), /setQueueHeld\(false\)/);
  // 배출 조건이 그 잠금을 본다(sendingNow도 — 지금 바로 보내기의 abort~재전송 창에서 대기열이
  // 먼저 발사되지 않게, 재검수 MEDIUM 2026-09-24)
...
test('업로드가 끝나기 전에는 보내지 않는다 — 첨부가 조용히 떨어진다(Enter 경로 포함)', () => {
  ...
  assert.match(send.slice(0, 900), /if \(uploading \|\| sendingNow\) return;/);
});
