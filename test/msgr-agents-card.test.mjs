...
test('카드: 생성·회전은 RPC(토큰은 응답에서 setup 상태로만) · 설정 두 줄 = URL+토큰 · 해제는 인라인 확인 · 표는 폐기 제외', () => {
  ...
  assert.match(card, /for \(const a of agents\) made\.push\(\{ \.\.\.\(await mkOrRotate\(kind, a\.name, `\$\{kind\}:\$\{a\.id\}`\)\)/, '에이전트마다 봇, 이름 = 에이전트 이름(종류 라벨 하드코딩 아님)');
...
test('원클릭 연결(유건 지시 "이렇게 어려우면 안 돼"): 앱 안에서 만들기/회전 직후 agent_connect(플러그인 설치·.env·게이트웨이)를 부르고, CLI가 없으면 수동 안내로', () => {
  ...
  assert.match(rs, /pub fn agent_list\(kind: String\)/, '에이전트 목록 커맨드'); assert.match(rs, /\("HERMES_HOME", hh\.display\(\)\.to_string\(\)\)/, '헤르메스 프로필 홈 격리');
