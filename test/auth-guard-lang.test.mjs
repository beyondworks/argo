...
test('배선 — app/·middleware의 authError 호출부 전 형태 강제(코드 실존 + 언어 전달로 허용 목록) + guardCompany 네 갈래', async () => {
  ...
  for (const f of files) {
    ...
    // 속 //는 그 줄 뒤쪽을, 문자열 속 /*('runners/*' 글롭 류)는 다음 */까지를, 블록 주석 속
    // //가 */를 지우면 그 너머를 가린다(음성 검사 fail-open·양성 검사 red). 반대로 ':' 바로 뒤
    // 주석(case 1:// 류)은 안 잘려 주석 텍스트가 남는다(양성 검사 거짓 충족 방향) — 전부 파서
    // 없는 근사의 문서화된 한계다.
    const stripped = src
