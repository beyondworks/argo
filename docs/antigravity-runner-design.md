...
1. **실계정 실턴 0회** — 이 맥에 agy 로그인이 없다. 유건 로그인 또는 제보자(`38e5281d`) 베타 확인이 관문.
   - 2026-07-28 재확인: 여전히 미로그인(`agy -p` 5분 후 `Error: timeout waiting for response` — 위 표의
     미로그인 시그니처 재현). **검증 하네스는 준비 완료** — `scripts/e2e-antigravity.mjs`(격리 서버 →
     host 옵트인 → 영입 턴 → 채팅 턴, `E2E_AGY_SMOKE=1`이면 카탈로그 8종 id를 agy 직호출로 스모크).
     agy Google 로그인 후 한 명령으로 이 항목을 지울 수 있다.
   - 같은 날 부분 실행 실측(미로그인 상태 그대로 돌림): 격리 서버에서 host 옵트인(낙관 authed)·회사
     생성까지 통과 후 영입 턴이 500으로 죽으며 **H1b 매핑 문구가 실바이너리 경로에서 그대로 나왔다**
     ("Antigravity가 제한 시간 안에 응답하지 않았습니다. … agy를 실행해 Google 로그인 후 다시 시도 …"
     — 타임아웃·미로그인 두 원인 병기). 즉 미로그인 → 로그인 안내 번역 배선은 풀스택 실측 완료.
