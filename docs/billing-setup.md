...
## 배포 순서 추가 (M2 — is_pro ends_at 집행, 2026-07-30)

`20260730050000_is_pro_ends_at.sql`은 통상 규칙("마이그레이션 먼저")의 **역방향**이다: 클라이언트
반쪽(fetchPlan ends_at 미러 + sync free 파일왕복 스킵, 같은 커밋)이 안전장치라서, 이 마이그레이션이
클라이언트보다 먼저 가면 구버전 데스크톱의 만료-pro 사용자에게 "업로드 거부 + 삭제 전파"의 단조 감소
비대칭(20260723 꼬리 주석의 그 시나리오)이 재현된다. **이 커밋을 담은 데스크톱 빌드가 배포 기준선이
된 뒤에 적용**한다. 현재 영향 인구는 0(ends_at 전부 null — LS 스토어 미가동)이라 즉시 적용도 무해하지만,
LS 스토어 가동 전에 클라이언트 확산이 선행돼야 한다는 순서 자체는 지켜라.
