...
- **Phase 1 — 슬림화·하드닝**: 24h 워커 경로 비활성/격리 · **페이월 서버측 이전**(§4-2) · **서비스롤 클라이언트 제거 완주**(§4-1) · M-ENC-1(기억 봉투 암호화 — 클라우드 동기화 GA 전 필수, `security-encryption-roadmap.md`) · **러너 연결 하드닝**(§6 — PATH·프로비저닝 검증 + 실패 진단).
