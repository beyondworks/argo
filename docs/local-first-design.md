...
5. **버전 뱃지 잔여** — 웹은 업데이트 신호 상실, `useAppUpdate`가 layout·settings에서 2벌 인스턴스화(타이머 중복), 마운트 자동 check 실패가 사용자 조작 없이 붉은 에러로 노출. (`/api/version` 죽은 라우트는 2026-07-23 전수 검수 후속으로 제거함. 페이월 pre-flight의 서비스 모드 판정도 같은 검수에서 ensureClient와 동일 조건으로 정렬 — §4번 drift 항목의 한 사례를 해소)
...
**남은 권고(미착수·비차단)**:
1. 활성화 원자성에 **코드 게이트**(RLS 거부 감지 시 `status.paywalled`로 승격해 삭제 전파까지 정지) — 주석 경고만으로 부족하다는 지적.
2. **`cycle()`의 순서 불변식이 테스트로 안 잠긴다** — `cycle`이 미export라 `renewLease` 호출을 요금제 게이트 아래로 되돌려도 전 스위트가 통과한다. 배선 테스트를 낳은 것과 **동일 계열의 갭**이므로, 재발 시 같은 버그가 조용히 돌아온다. `cycle` 테스트 export 또는 순서 스파이 훅 권장.
