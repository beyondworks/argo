...
import { externalExec, glmEnv, GLM_DEFAULT_MODEL, RUNNERS, claudeEnvFor } from './runners.mjs';
...
function systemPromptFor(cardMd, wsRoot, skills, meta = {}) {
...
${systemPromptFor(md, p.root, skills, meta)}
${ctx ? `\n## 최근 대화\n${ctx}\n` : ''}
...
${caps.bypass ? '- 권한 우회 모드: 켜짐 — 결재 없이 실행되니 되돌릴 수 없는 명령은 스스로 한 번 더 확인하라' : '- 부작용 있는 실행은 결재 승인 후 이어진다 — 승인 대기는 정상 흐름이다'}`;
  ...
  // 회사 Claude 키(BYOK) — 있으면 이 턴의 SDK 자격으로 주입한다. 없으면 null → 기존 CLI/env 자격 폴백(회귀 0).
  const claudeEnv = runner === 'claude' ? await claudeEnvFor(wsId) : null;
  await setTurnStatus(wsId, agentSlug, '시동 거는 중'); // 즉시 — SDK 부팅 전에도 살아있음을 보인다
  const q = query({
    prompt: promptInput,
    options: {
      cwd: p.root,
      systemPrompt: systemPromptFor(md, p.root, skills, meta)
...
- 초안 작성·분석·vault 기록 같은 회사 안 작업은 결재 없이 바로 한다.`
      ...
      ...(claudeEnv ? { env: claudeEnv } : {}), // 회사 Claude 키 — claude 러너에만, 키 있을 때만(글루 러너 env와 상호배타)
      ...(caps.bypass
