...
function makeCrewServer(wsId, fromSlug, fromName, colleagues, hop = 0, chain = [], mirrorCtx = null, lang = 'ko') {
  ...
  const requestApproval = tool(
    ...
    async ({ action, reason }) => {
      ...
      // 승인 채널 헬스 — 메신저가 설정돼 있는데 죽어 있으면 사장이 버튼을 못 받는다.
      // 크루가 그 사실을 알고 웹 결재함으로 안내하게 한다(실측: 봇 사망 중 버튼만 기다리다 데드락).
      let channelNote = '';
      try {
        ...
        if (conn.telegram.enabled && conn.telegram.token && !st.telegram.alive) {
          channelNote = ' 주의: 지금 텔레그램 연결이 응답하지 않아 승인 버튼이 안 갈 수 있다 — 사장에게 웹 결재함에서 승인해 달라고 함께 안내하라.';
        }
      } catch { /* 헬스 확인 실패는 결재 등록을 막지 않는다 */ }
      return text(`결재 요청이 등록되었다(${item.id}). 승인 전에는 절대 그 행동을 실행하지 마라. 지금은 "결재를 올렸고 승인되면 진행하겠다"고 사용자에게 알리고 턴을 마무리하라.${channelNote}`);
    },
  );
  ...
  const requestToolInstall = tool(
    ...
    async ({ source, id, why }) => {
      ...
      const item = await addApproval(wsId, {
        slug: fromSlug, kind: 'mcp',
        action: `도구 설치: ${cleanId} (${source === 'host' ? '이 컴퓨터에서 가져오기' : '카탈로그'})`,
        reason: why, payload: { source, id: cleanId },
      });
      return text(`설치 결재가 등록되었다(${item.id}). 승인되면 시스템이 설치하고 후속 지시가 온다. 지금은 사장에게 승인을 요청하고 턴을 마무리하라. 승인 전에 그 도구를 쓰려 하지 마라.`);
    },
  );
...
${caps.bypass ? '- 권한 우회 모드: 켜짐 — 결재 없이 실행되니 되돌릴 수 없는 명령은 스스로 한 번 더 확인하라' : '- 부작용 있는 실행은 결재 승인 후 이어진다 — 승인 대기는 정상 흐름이다'}`;
  ...
  // 회사 환경 설명서 — 크루가 자기 환경을 몰라 헤매던 실측 사례(호스트 .claude 설정을 뒤지고,
  // 결재 만료를 실패로 보고, 권한 우회 모드의 존재를 몰라 안내 못 함)를 막는 자기 지식.
  const connectedMcp = Object.keys(servers ?? {});
  const envGuide = lang === 'en'
    ? `\n## Your environment (Argo) — guide the captain precisely when blocked
- You work inside an Argo company. External tools (MCP) are connected PER COMPANY in the "Skills·Tools" screen — this runtime does NOT inherit the computer's Claude Code config (.claude.json, .mcp.json) by design (tenant isolation). Never hunt for those files.
- Tools connected to this company now: ${connectedMcp.length ? connectedMcp.join(', ') : '(none)'}. If a task needs a missing tool, use the request_tool_install tool (a catalog id, or the name of an MCP already on this computer) — the captain approves and the system installs it.
...
- If frequent approvals interrupt the flow, you may suggest the captain enable "permission bypass mode" (bottom of Settings — trusted companies only).`
    : `\n## 너의 환경(Argo) — 막혔을 때 사장에게 정확히 안내하라
- 너는 Argo 회사 안에서 일한다. 외부 도구(MCP)는 **회사별로** "스킬·도구" 화면에서 연결된다 — 이 런타임은 컴퓨터의 Claude Code 설정(.claude.json, .mcp.json)을 설계상 상속하지 않는다(테넌트 격리). 그 파일들을 찾아 헤매지 마라.
- 지금 이 회사에 연결된 도구: ${connectedMcp.length ? connectedMcp.join(', ') : '(없음)'}. 작업에 필요한 도구가 없으면 request_tool_install 도구로 설치를 결재 요청하라(카탈로그 id 또는 이 컴퓨터에 이미 있는 MCP 이름) — 사장이 승인하면 시스템이 설치한다.
- 결재는 웹 결재함 또는 텔레그램/슬랙 버튼으로 승인된다. 대기 시간이 지나도 **실패가 아니다** — 요청은 결재함에 남고, 사장이 나중에 승인하면 후속 턴에서 이어서 실행된다.
- 승인이 잦아 흐름이 끊기면 사장에게 "권한 우회 모드"(설정 맨 아래 — 신뢰하는 회사 전용)를 안내할 수 있다.`;
