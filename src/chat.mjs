...
import { externalExec, GLM_DEFAULT_MODEL, RUNNERS, sdkEnvFor, runnerCredEnv, runnerStatus } from './runners.mjs';
...
function systemPromptFor(cardMd, wsRoot, skills, meta = {}, lang = 'ko') {
  ...
  // 출력 언어 — 회사 설정(company.lang). en일 때만 명시 지시(ko는 기존 창발 유지 = 회귀 0).
  // 카드·골격이 한글이어도 답변 언어를 강제한다. 맨 앞에 둬 모델이 먼저 보게 한다.
  const langLine = lang === 'en'
    ? `## Output language — highest priority\n- Always respond to the captain in natural, professional English, regardless of the language of the persona card or the instructions below. Never mix Korean into your reply.\n\n`
    : '';
  return `${langLine}${cardMd}
...
function makeCrewServer(wsId, fromSlug, fromName, colleagues, hop = 0, chain = [], mirrorCtx = null) {
...
${lang === 'en'
  ...
  const crewServer = makeCrewServer(wsId, agentSlug, meta.name || agentSlug, colleagues, hop, chain, mirrorCtx);
...
- 사장이 크루 프로필(이름·역할·팀·규칙·러너·모델) 변경이나 새 크루 영입을 요청하면 파일을 직접 고치지 말고 update_profile / hire_crew 도구로 결재를 올려라. 러너·모델이 정해지지 않았으면 카탈로그에서 선택지를 2~3개 제시해 물어본 뒤 올려라.`
      ...
      ...(sdkEnv ? { env: sdkEnv } : {}),
      ...(runner === 'glm' ? { model: meta.model || GLM_DEFAULT_MODEL } : (meta.model ? { model: meta.model } : {})),
      ...(caps.bypass
  ...
  for await (const msg of q) {
    ...
    if (msg.type === 'result') {
      ...
      await appendUsage(wsId, {
        kind: from ? 'delegate' : (source ?? 'chat'), slug: agentSlug, from, runner, model: actualModel || meta.model || null,
