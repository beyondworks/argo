...
## 로컬 능력 — 전권
- 파일 시스템: ${isCliRunner(runner) ? (runner === 'kiro' ? '**홈 폴더 전체**(바탕화면·문서·기존 프로젝트 폴더 포함)와 아래 보호 구역 밖의 모든 경로를 읽고 쓸 수 있다. 이 러너(Kiro)는 지정 작업 폴더가 반경으로 적용되지 않으니, 폴더를 등록해야 열린다고 안내하지 마라 — 보호 구역 밖 경로가 존재하면 그대로 쓰면 된다' : `**홈 폴더 전체**(바탕화면·문서·기존 프로젝트 폴더 포함)와 아래 지정 작업 폴더를 읽고 쓸 수 있다. 켜야 할 토글은 없다. 홈 밖 경로(외장 볼륨 등)가 필요하면 사장에게 "설정 → 작업 폴더"에 그 폴더를 등록해 달라고 안내하라 — 등록하면 다음 턴부터 열린다${runner === 'kiro' ? '. 단, 이 러너(Kiro)는 지정 작업 폴더가 반경으로 적용되지 않는다 — 아래 보호 구역 밖은 이미 접근 가능하니 "등록하면 열린다"고 안내하지 마라' : ''}${runner === 'gemini' ? '. 단, 구버전 Gemini CLI는 벤더 제한으로 회사 폴더 밖이 그래도 막힐 수 있다 — 막히면 권한 추측 없이 원인 오류를 그대로 보고하고, 결과물은 회사 폴더에 저장해 위치를 알려라' : ''}`) : '이 컴퓨터 어디든 읽고 쓸 수 있다. 사장의 바탕화면·문서·기존 프로젝트 폴더 전부 포함이다. 켜야 할 토글도, 사장을 보낼 메뉴도 없다 — 경로가 존재하면 그대로 쓰면 된다'}. 막히는 것은 아래 보호 구역뿐이다.
...
export async function chat(wsId, agentSlug, userMsg, sessionId = null, { from = null, source = null, attachments = [], hop = 0, chain = [], toolHop = 0, mirrorCtx = null, runnerOverride = null, modelOverride = null, __freshRetry = false, __seedNotes = null, __excludeRunners = null, __crashRetry = false } = {}) {
  ...
  if (!resolved.available) {
    ...
    throw new Error(noCli.length
      ...
      : (lang === 'en'
          ? 'No AI runner is connected. Connect one in Settings → AI connections (Claude, Codex, Gemini, Antigravity, Kiro, GLM, Kimi, OpenRouter, or Grok), then try again.'
          : 'AI 러너가 하나도 연결돼 있지 않습니다. 설정 → AI 연결에서 Claude·Codex·Gemini·Antigravity·Kiro·GLM·Kimi·OpenRouter·Grok 중 하나를 연결한 뒤 다시 말을 걸어 주세요.'));
  }
