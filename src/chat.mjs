...
import { loadActiveWorkRoots, activePin } from './workroots.mjs';
...
export function commonDirectives({ caps = {}, connectedMcp = [], connectors = [], hasTools = true, lang = 'ko', runner = null, workRoots = [], pinnedFolder = '' } = {}) {
  ...
  const otherRoots = workRoots.filter((r) => r !== pinnedFolder);
  ...
  const pinnedLine = pinnedFolder
    ? (lang === 'en'
      ? `- **Work here now: ${pinnedFolder}** — the captain pinned this folder. Unless they name another path, do your file work inside it (create, read, save there) and say which folder you used. It stays pinned until they unpin it.\n`
      : `- **지금 일할 폴더: ${pinnedFolder}** — 사장이 고정해 둔 곳이다. 다른 경로를 지정받지 않는 한 파일 작업(생성·조회·저장)은 이 폴더 안에서 하고, 어느 폴더에 뒀는지 밝혀라. 사장이 고정을 풀기 전까지 유지된다.\n`)
    : '';
  const rootsLine = otherRoots.length
    ? (lang === 'en'
      ? `- Other folders you may use: ${otherRoots.join(' · ')}\n`
      : `- 그 밖에 써도 되는 폴더: ${otherRoots.join(' · ')}\n`)
    : '';
...
export async function chat(wsId, agentSlug, userMsg, sessionId = null, { from = null, source = null, attachments = [], hop = 0, chain = [], toolHop = 0, mirrorCtx = null, runnerOverride = null, modelOverride = null, __freshRetry = false, __seedNotes = null, __excludeRunners = null } = {}) {
  ...
  if (isCliRunner(runner)) {
    ...
    try {
      ...
      const cliPin = await activePin(wsId, agentSlug); // 고정 작업 폴더('' = 없음) — 러너 무관 같은 문구
...
${lang === 'en'
  ...
  const pinnedFolder = await activePin(wsId, agentSlug); // 고정 작업 폴더 — CLI 경로와 같은 출처·같은 문구(중립성)
