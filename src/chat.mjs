...
export function commonDirectives({ caps = {}, connectedMcp = [], hasTools = true, lang = 'ko' } = {}) {
...
- Shell commands: ${caps.shell ? 'allowed' : `off — ${offGuide}`}
...
- External tools (MCP) connected to this company: ${mcpList}. ${hasTools ? 'When the work calls for one, use it right away — don\'t ask permission to use what\'s already connected.' : 'These run on Claude/GLM (SDK) turns — if you can\'t use them on this runner, say so and offer an alternative.'}
...
- 셸 명령: ${caps.shell ? '허용' : `꺼짐 — ${offGuide}`}
...
- 이 회사에 연결된 외부 도구(MCP): ${mcpList}. ${hasTools ? '작업에 필요하면 허락을 기다리지 말고 바로 사용하라 — 그러라고 연결해 둔 것이다.' : '이 도구들은 Claude·GLM(SDK) 러너 턴에서 실행된다 — 지금 러너에서 쓸 수 없으면 그 사실을 밝히고 대안을 제시하라.'}
...
${systemPromptFor(md, p.root, skills, meta, lang)}${commonDirectives({ caps: cliCaps, connectedMcp: cliMcp, hasTools: false, lang })}${fallbackDirective}
