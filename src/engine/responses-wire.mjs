...
export function toResponsesRequest({ system, messages, tools, model, effort }) {
  const input = [];
...
export function finalFromSse(text) {
  ...
  if (t.startsWith('{')) { const j = JSON.parse(t); if (j?.error) throw Object.assign(new Error(`API Error: 400 ${extractErrorMessage(t)}`), { status: 400 }); return j.response ?? j; }
  ...
  if (failed) throw Object.assign(new Error(`API Error: 400 ${failed}`), { status: 400 });
