...
/** CLI 파일 하나에서 토큰을 읽는다(쓰기 금지; refresh 없으면 반입 안 함). 액세스 토큰 만료는 무관 — 반입 목적은 refresh 토큰이다. */
export async function readCliCodexAuth(file = join(homedir(), '.codex', 'auth.json')) {
  try { const t = parseCodexAuth(await readFile(file, 'utf8')); return t && t.refresh_token ? t : null; } catch { return null; }
}
/** 반입 후보 순서 — Argo의 codex CLI 턴은 호스트 ~/.codex가 아니라 격리 홈(~/.argo/codex-home-<wsId>)을 쓰고 그 CLI가 토큰을 회전시킨다(seedAuthFile adopt 주석).
    회사 자격의 refresh 토큰이 이미 회전돼 죽은 흔한 경우, 더 새 토큰은 격리 홈에 있다(분리 검수 MEDIUM-3). 호스트 ~/.codex는 그 다음. */
export const cliAuthCandidates = (wsId, home = homedir()) => [join(home, '.argo', `codex-home-${wsId}`, 'auth.json'), join(home, '.codex', 'auth.json')];
