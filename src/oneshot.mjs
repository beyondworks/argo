...
export async function runOneShot(wsId, prompt, opts = {}) {
  ...
  if (!resolved.available) {
    // 자가치유 제외 끝이면 사실대로 — "하나도 연결돼 있지 않습니다"는 거짓(chat.mjs와 동일, Grok 제보 2026-08-06)
    if (__exclude?.length) throw new Error(authExcludedNoRunnerMsg(__exclude, lang));
    ...
    throw new Error(noCli.length
      ...
      : (lang === 'en'
          ? 'No AI runner is connected — connect Claude, Codex, Gemini, Antigravity, Kiro, GLM, Kimi, OpenRouter, or Grok in Settings → AI connections.'
          : 'AI 러너가 하나도 연결돼 있지 않습니다 — 설정 → AI 연결에서 Claude·Codex·Gemini·Antigravity·Kiro·GLM·Kimi·OpenRouter·Grok 중 하나를 연결해 주세요.'));
  }
