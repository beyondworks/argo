...
export async function appendTurn(wsId, slug, { turnId, userMsg, reply, handover, sessionId, attachments, artifacts, via, actor, failed, aborted, fellBack }) {
  return withLock(lockKey(wsId, slug), async () => {
    ...
    t.messages.push(
      ...
      { who: 'user', text: userMsg, ts, ...(attachments?.length ? { attachments } : {}), ...(via ? { via } : {}), ...(actor ? { actor } : {}), ...(failed ? { failed } : {}), ...(aborted ? { aborted: true } : {}) },
    );
