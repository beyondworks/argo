...
export async function appendTurn(wsId, slug, { turnId, userMsg, reply, handover, sessionId, attachments, artifacts, via, actor, failed, aborted, cancellationIncomplete, viaSendNow, fellBack, failedCode, failedOrigin, modelFallback, contextScope }) {
  return withLock(lockKey(wsId, slug), async () => {
    ...
    if (at >= 0) {
      ...
      // viaSendNow — 이 중단이 "지금 바로 보내기"가 건 것(재검수 D) — UI가 정지 버튼 문구·재전송
      // 버튼과 갈라 그린다. 새로고침·폴링 병합 뒤에도 남아야 해서(D 지적) 서버에 저장한다.
      if (viaSendNow) m.viaSendNow = true;
    ...
    t.messages.push(
      ...
      { who: 'user', text: userMsg, ts, ...scoped, ...(attachments?.length ? { attachments } : {}), ...(via ? { via } : {}), ...(actor ? { actor } : {}), ...(failed ? { failed } : {}), ...(failedCode ? { failedCode } : {}), ...(failedOrigin ? { failedOrigin } : {}), ...(aborted ? { aborted: true } : {}), ...(cancellationIncomplete ? { cancellationIncomplete: true } : {}), ...(viaSendNow ? { viaSendNow: true } : {}) },
    );
