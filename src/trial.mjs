...
import { resolveRunner } from './runners.mjs'; // 실패 원인 판별 — "연결 없음"과 "연결됐는데 실행 실패"를 구분
  ...
  (async () => {
    try {
      ...
      const reply = connected
        ...
        : 'AI 연결이 아직 안 되어 있어요. 설정 → AI 연결에서 Claude·Codex·Gemini·Antigravity·GLM·Kimi·OpenRouter 중 하나를 연결하면 바로 일을 시작할게요.\n\nAI isn\'t connected yet. Connect any runner (Claude, Codex, Gemini, Antigravity, GLM, Kimi, or OpenRouter) in Settings → AI connections and I\'ll get started.';
      await appendTurn(wsId, slug, {
        userMsg: '(영입 시운전) 첫 인사와 샘플 산출물을 보여주세요.',
