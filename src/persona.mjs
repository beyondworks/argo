...
export async function createAgentFromPrompt(wsId, oneLiner, { name, team } = {}) {
  ...
  if (!md) {
    throw new Error(lang === 'en'
      ? 'AI connection is needed — connect any runner (Claude, Codex, Gemini, Antigravity, GLM, Kimi, or OpenRouter) in Settings → AI connections to hire.'
      : 'AI 연결이 필요합니다 — 설정 → AI 연결에서 아무 러너나(Claude·Codex·Gemini·Antigravity·GLM·Kimi·OpenRouter) 연결하면 영입할 수 있어요.');
  }
