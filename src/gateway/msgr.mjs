...
export const HEARTBEAT_WRITE_MS = 60_000; // 심박 쓰기 최소 간격(판정 90초 − 틱 15초 이내)
...
export function makeDb(client) {
  return {
    ...
    async heartbeat(ids) {
      // 60초 넘게 지난 행만 쓴다 — 15초 틱마다 모든 크루 행을 갱신해 msgr_crews가 분당 1,335행씩 다시 써졌다(2026-09-23 DB 점검).
      // 부재중 판정은 전부 90초(앱 AWAY_MS·work_runs), 알림 경로 3분 — 최대 75초라 표시가 바뀌지 않는다.
