...
> 생성일 2026-07-10. 이 파일은 루트 `lean-projects/AGENTS.md`와 글로벌 `~/.Codex/AGENTS.md`를
...
- 스택: Next.js(App Router, JS) + Codex Agent SDK 코어(`src/*.mjs`) — P1에서 Supabase 인증·DB 합류
- 배포처: 로컬 dev(현 단계) → Fly.io/Railway 워커 (P1)
- 외부 통합: Codex Agent SDK(BYOK — API 키/구독 OAuth 환경변수), MCP(후속)
...
- 한국어 모드 = 고유명사(Argo, Codex 등) 제외 전부 한글. 마이크로라벨도 한글.
